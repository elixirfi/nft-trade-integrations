...
import { AnchorState, composeProgram, FEE_PID, getExtraComputeTxn, getLookupTable, getMetadataKey, getProgramsLookupTable, NftMetadata, PriorityFee, PROGRAM_IDS, SELL_TXN_FEES, TokenStandard, TOTAL_RAYDIUM_FEE, TransactionResult, TREASURY, V0_COMPUTE_INCREASE, V0_PRIORITY_FEE, WhirlpoolElixirData } from "../../utils";
...
export const elixirOrcaSell = async (
    ...
    // subtract 1 lamport to fix rounding errors
    const postFee = preFee - 1 / LAMPORTS_PER_SOL;
    ...
    const sellIx = await compose.methods
        .orcaSell(nonce, new BN(postFee * LAMPORTS_PER_SOL), new BN("79226673515401279992447579055"), doSwap)
        .accounts({
